<!doctype html>
<html lang="en">
  <head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">

    <title>BMICalculator</title>
  </head>
  <body>
  <div class="container">
     <!--bootstrap card-->
       <div class="card m-4">
         <!--card-header -->
         <div class= "card-header bg-info">
          <h1 class="text-white"> BMI CALCULATOR </h1>
         </div>
         <div class="card-body">
           <div class="row">
            <div class="col-6">
                <form method="get">
                <div class="form-group">
                <label for="wgt">weight</label>
                <input type="number" name="wgt" placeholder="weight" class="form-control">
                </div> 
                <div class="form-group">
                <label for="hgt">height(in cm)</label>
                <input type="number" name="hgt" placeholder="height" class="form-control">
                </div> 
                <input type="submit" value="calculate" class="btn btn-info btn-lg">
            </form>
            </div>
            <div class="col-6 text-center">
            <p class=" display-3">
            <?php 
                    if($_GET){
                    $wgt=$_GET['wgt'];
                    $hgt=$_GET['hgt'];
                    
                    
                    $hgt=$hgt/100;
                    //echo  "<h3>".$wgt." ".$hgt."</h3>";

                    $bmi=$wgt /($hgt*$hgt);
                   // echo "<h2>your BMI is $bmi</h2>";
                   switch($bmi){
                       case ($bmi<18.5):
                        echo "underwight-$bmi";
                        break;
                        case ((18.5<$bmi) && ($bmi<24.9)):
                                echo "normal-$bmi";
                                break;
                            case ((24.9<$bmi) && ($bmi<24.9)):
                                    echo "overweight-$bmi";
                                    break;
                            case  ((29.9<$bmi) && ($bmi<35)):
                                   echo "obese-$bmi";
                                   break;
                            default:
                              echo "Extremley obese - $bmi";

                   }
                    }
                    ?>
            </p>
            </div>
         </div>
       </div>
       

 </div>
    <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
  </body>
</html>

